...
Generate + start UI in one step:
```bash
python3 scripts/generate_topology.py --scenario scenarios/base_case_30.yaml --start-ui
```
...
- Legitimate announcement: AS1 announces `192.168.1.0/24`
...
- `configs/frr2.conf` AS2 with connection to censor, doesn't change the structure of the files
...
```bash
# You should see "connected to peer-1 with outgoing-1 10.0.5.2-10.0.5.1". This means that the connection between the censor and AS2 is successfully established.
sudo docker logs clab-bgp-testbed-exabgp-censor
```

#### 2. Check BGP Session
```bash
# You can see that the censor AS 65999 is also part of the neighbors of AS 2
sudo docker exec -it clab-bgp-testbed-AS2 vtysh -c "show ip bgp summary"
```

#### 3. Verify Route Hijacking
```bash
# Shows routes from AS3 to the network 192.168.1.0/25. This shows you, if the BGP-session to the censor works.
sudo docker exec -it clab-bgp-testbed-AS3 vtysh -c "show ip bgp 192.168.1.0/25"
```
...
sudo docker exec clab-bgp-testbed-AS3 ping 192.168.1.10 -I 192.168.3.1 -c 4
```
